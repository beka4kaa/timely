...
# ─────────────────────────────── Удаление ────────────────────────────────────
#
# `CourseEnrollment.version` — FK с `PROTECT`, и это правильно: версия, по
# которой ученик занимается, не должна исчезать под ним из-за постороннего
# удаления. Но когда ученик сам удаляет свой предмет, каскад доходит до этой
# защиты и падает `ProtectedError` — снаружи это выглядело как 500 на кнопку
# «Удалить предмет».
#
# Поэтому записи о занятиях снимаются явно и первыми. Это не обход защиты, а
# признание того, что удаление собственного предмета — намеренное действие, а
# не побочный эффект: `CourseEnrollment.plan` и так `CASCADE`, то есть запись
# всё равно уходит вместе с планом. Мы лишь снимаем её раньше, чем коллектор
# успевает упереться в защищённую версию.


def delete_goal(goal: LearningGoal) -> None:
    """Удаляет предмет вместе с книгами, планами и записями о занятиях."""
    with transaction.atomic():
        CourseEnrollment.objects.filter(plan__goal=goal).delete()
        goal.delete()


def delete_plan(plan: CoursePlan) -> None:
    """Удаляет программу вместе с записью о занятиях по ней."""
    with transaction.atomic():
        CourseEnrollment.objects.filter(plan=plan).delete()
        plan.delete()
