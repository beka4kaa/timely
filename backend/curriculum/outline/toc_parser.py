...
def _clean_title(text: str) -> str:
    """Убирает выноски, номер страницы и лишние пробелы.

    Хвостовое число срезается ТОЛЬКО когда выносок не было. С выносками номер
    страницы уже удалён, и второй проход съедал бы номер самой записи:

        'Упражнение 1 . . . . . . . 56'  →  'Упражнение 1'   (а не 'Упражнение')

    Все шестнадцать упражнений книги превращались в одинаковое «Упражнение», и
    отличить их в плане было невозможно.
    """
    cleaned, had_leaders = _TAIL.subn("", text)
    if not had_leaders:
        # Запись без выносок — «Название   57». Здесь хвостовое число и есть
        # номер страницы.
