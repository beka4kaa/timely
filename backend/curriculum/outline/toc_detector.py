...
# Параграф, чьё название заняло всю ширину строки: типограф убрал выноски, и
# перед номером остался ОДИН пробел.
#
#     § 9.4. Пластичность и хрупкость . . . . . . . . . . . 448   ← _ENTRY
#     § 9.5. Давление в жидкостях и газах. Сообщающиеся сосуды 450 ← сюда
#
# Одного пробела мало, чтобы отличить номер страницы от последнего слова
# названия, поэтому шаблон сужен до строк с номером параграфа. Проверено на
# «Механике» Мякишева: без сужения строка «Глава 1» разбирается как запись со
# страницей 1, все части книги схлопываются в один уровень и вместо 5 частей и
# 12 глав получается 1 и 16.
_ENTRY_PARAGRAPH = re.compile(
    r"^\s*§\s*[\d.]+\.?\s+(?P<title>\S.*?[^\d\s])\s+(?P<page>\d{1,4})\s*$"
)
...
def entry_page_number(line: str) -> int | None:
    ...
    for pattern in (_ENTRY, _ENTRY_LOOSE, _ENTRY_PARAGRAPH):
