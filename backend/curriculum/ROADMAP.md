...
- scoped backend: **535 тестов OK** (`curriculum`), **977** вместе с `ai_engine`;
  один настоящий PostgreSQL concurrency-test пропускается на SQLite
...
8. **Планировщик курса — `minimax/minimax-m3`, рецензент — `deepseek/deepseek-v4-flash`.**
   До этого роль `COURSE_PLANNING` вообще не была привязана к модели и молча
   откатывалась на `TEXT_LLM_MODEL`, из-за чего все её настройки не читались.
   Рассматривавшийся `deepseek/deepseek-r1` отпал по потолку вывода: 16 000
   токенов, тогда как программа на 12 модулей занимает 7.9–11.2 тыс. и это без
   запаса. Рецензент оставлен на дешёвой модели намеренно — это второй
   последовательный вызов, и он лишь сверяет готовый план по чеклисту.
9. **Форму ответа задаёт JSON Schema, а не проза промпта.** Strict structured
   outputs снимают класс «модель прислала синоним вместо значения enum» до
   генерации, а не после неё. Таблицы синонимов (`normalize_enum_fields`) и
   `validate_plan` при этом ОСТАЮТСЯ: роль может быть переставлена на модель без
   strict-поддержки, fake- и fixture-провайдеры схему не проходят вовсе, а
   галлюцинированные `source_chunk_ids`, циклы и покрытие схема не проверяет в
   принципе. Схема, парсер и валидатор связаны тестами на расхождение
   (`CoursePlanSchemaTests`).
10. **Провайдер выбирается списком, а не флагом `require_parameters`.** См. грабли
    ниже: флаг сужает пул, но уводит на самого медленного из подходящих. Список
    задаётся через `COURSE_PLANNING_PROVIDERS`, а не зашит в код, — состав
    провайдеров у OpenRouter меняется.
...
- **Настроенный `.env` протекает в тесты — ЧЕТЫРЕЖДЫ одна и та же болезнь.**
  ...
  Четвёртым стал `COURSE_PLANNING_MODEL`: как только он появился в `.env`, роль
  стала «настроенной», и `get_planning_provider` начал отдавать сетевой провайдер
  вместо fake — то есть любой тест генерации плана, не подставивший провайдера
  явно, пошёл бы в сеть и в оплату. Рубильник здесь не флаг, а снятие самих
  переменных: `model_registry.resolve_model` читает окружение НАПРЯМУЮ, мимо
  settings, поэтому при `"test" in sys.argv` из `os.environ` убираются
  `COURSE_PLANNING_MODEL`, `COURSE_PLANNING_PROVIDERS`, `COURSE_REVIEW_MODEL` и
  `TEXT_LLM_MODEL`.
  Рубильников четыре: `CURRICULUM_EMBEDDINGS_ENABLED`, `CURRICULUM_S3_ENABLED`,
  обнуление `CELERY_BROKER_URL` и снятие ролевых `*_MODEL`. **Заводя пятый внешний
  сервис, заводи и пятый рубильник — иначе тесты сломаются не сразу и не
  очевидно.**
...
- **Починка плана может выбить генерацию за таймаут gunicorn.** `_plan_with_one_repair`
  ...
  случай — около 360 с плюс поиск контекста, тогда как в проде стояло
  `gunicorn --timeout 200`: воркер убивался по SIGABRT посреди запроса, и ученик
  получал оборванное соединение вместо плана. Наблюдалось локально: генерация не
  уложилась и в 180 с клиентского таймаута.
  **Смягчено** общим дедлайном `CURRICULUM_PLAN_DEADLINE_SECONDS` (300 с) на всю
  генерацию: шаги, на которые времени не осталось, пропускаются с записью в
  `outcome.warnings`, а не роняют запрос. Порядок слоёв теперь такой и его нельзя
  нарушать: модель 180 с < дедлайн 300 с < `gunicorn --timeout` 360 с <
  клиент 420 с. **Настоящее решение по-прежнему — перевести генерацию плана на ту
  же схему «задача + опрос», что и обработку учебника** (Фаза 4b): дедлайн не
  устраняет ни дубли планов, ни бесполезную работу после обрыва клиента.
- **`provider.require_parameters` у OpenRouter — не бесплатная гарантия.** Он
  сужает пул до провайдеров, поддерживающих все параметры запроса, но НЕ выбирает
  среди них лучший. Замерено на `minimax/minimax-m3` (один и тот же запрос,
  12 модулей): с `require_parameters` маршрут уходил на Morph — 175 / 178 / 244 с,
  и Together не выбирался ни разу, даже при явном `order: ["together"]`. Без него,
  но с явным списком тех же самых strict-провайдеров и `sort: "throughput"` тот же
  запрос отработал за 57 с на Together. Поэтому роль пришпилена списком
  `COURSE_PLANNING_PROVIDERS`, а не флагом. Ловушка общая: в листинге
  `/api/v1/models` поле `supported_parameters` — это ОБЪЕДИНЕНИЕ по провайдерам;
  реальную поддержку надо смотреть в `/endpoints` (у m3 strict умеют трое из
  девяти). Это тот же урок, что и с доступностью image-моделей.
- **`reasoning.effort` у m3 не тратит бюджет, но программа тратит.** Во всех живых
  прогонах `reasoning_tokens` = 0, то есть исходная гипотеза «effort съест
  max_tokens» не подтвердилась. Лимит всё равно поднят с 8000 до 24000 по другой
  причине: программа на 12 модулей занимает 7.9–11.2 тыс. токенов вывода, и
  прежние 8000 её обрезали бы, а обрезанный JSON — это полностью выброшенный
  платный вызов.
- **Резервация квоты считается по ПОТОЛКУ вывода, а не по факту.**
  `provider_call_reservation` бронирует `вход + max_output_tokens`
  (`usage.provider_call_token_upper_bound`), и бронь проверяется ДО вызова. Подъём
  `COURSE_PLANNING_MAX_TOKENS` с 8000 до 24000 поэтому бьёт по допуску, а не по
  счёту: на живой проверке вызов с почти пустым входом был отклонён с
  `AIUsageLimitExceeded (five_hour)`, потому что 24000 + вход уже не влезают в
  `AI_FREE_TOKEN_5H_LIMIT` = 25000. **Следствие: на тарифе Free построить программу
  нельзя в принципе**, и снижением лимита это не лечится — программа на 12 модулей
  занимает 7.9–11.2 тыс. токенов, то есть 8000 её просто обрежет. Нужно либо явно
  сделать построение программы платной функцией с понятным сообщением, либо
  поднять Free-лимит. Сейчас ученик увидит общую ошибку про исчерпанный лимит.
- **httpx-таймаут — это простой между чанками, а не дедлайн вызова.** Провайдер,
  шлющий keep-alive пока модель думает, сбрасывает таймер на каждом чанке
  (замерено на `planning_intake`: настроено 20 с, фактически ~120 с). Жёсткий
  wall-clock backstop живёт теперь в `text_llm.generate_json_content` и действует
  для всех вызывающих. При этом **`ThreadPoolExecutor` НЕ наследует `ContextVar`**:
  без явного `contextvars.copy_context()` расход писался бы анонимным, потому что
  `record_model_usage` берёт e-mail и feature именно из контекста. Ровно этим
  болел прежний собственный executor в `planning_intake`.
