...
def estimate_topic_minutes(
    *,
    ...
    `page_count` — сколько страниц книги покрывает тема. Ноль означает, что
    границы раздела не установлены; тогда возвращается умолчание, а не ноль:
    тема без времени ломает прогноз сроков.
    """
