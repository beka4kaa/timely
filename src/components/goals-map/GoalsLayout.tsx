...
export function GoalsLayout() {
        ...
        {VIEWS.map(v => {
          ...
          return (
            <button
              ...
              className={cn(
                'inline-flex items-center justify-center gap-1.5 min-w-[60px] h-8 px-4 rounded-full text-[13px] font-medium transition-colors',
                active
  ...
  const header = (
    <div className="flex items-start justify-between gap-4">
      <div>
        <h1 className="font-plus-jakarta text-3xl font-extrabold tracking-tight">Карта целей</h1>
        <p className="text-muted-foreground text-sm mt-1">Планируй цели по месяцам и связывай их в граф</p>
      </div>
      {controls}
    </div>
  )
  ...
  return (
    <div className={`max-w-5xl mx-auto w-full p-6 flex flex-col gap-5${showYearGate ? ' overflow-hidden' : ' pb-12'}`}>
      {header}
          ...
          >
            ...
            <section className={cn(GLASS, 'rounded-[20px] p-3 w-full max-w-2xl')}>
              <div className="grid grid-cols-[150px_1fr] gap-3">
                ...
                <div className="border-l border-foreground/[0.07] pl-3">
