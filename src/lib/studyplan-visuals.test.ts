...
test("время занятия печатается диапазоном, а не одним началом", () => {
  // «09:00 · 2 ч» заставляло считать конец в уме, а именно конец ученик и
  // проверяет, когда прикидывает, успеет ли он на секцию.
  assert.equal(timeRangeLabel(9 * 60, 120), "09:00–11:00 · 2 ч");
  assert.equal(timeRangeLabel(16 * 60 + 15, 45), "16:15–17:00 · 45 мин");
});

test("занятие через полночь не печатает 24-й час", () => {
  assert.equal(timeRangeLabel(23 * 60 + 30, 60), "23:30–00:30 · 1 ч");
});
