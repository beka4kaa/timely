 ...
 *
 * Должно быть ЗАМЕТНО больше бэкендового таймаута модели (`COURSE_PLANNING_TIMEOUT`,
 * по умолчанию 180 с) плюс поиск контекста и валидация. Равные таймауты — худший
 * вариант: клиент обрывает запрос ровно тогда, когда сервер ещё мог ответить, и
 * пользователь видит ошибку вместо готового плана.
 *
 * Верхняя граница остаётся проблемой: при починке плана бэкенд вызывает модель
 * ДВАЖДЫ, и худший случай не укладывается ни в один разумный таймаут. Настоящее
 * решение — перевести генерацию на ту же схему «задача + опрос», что и обработку
...
function toWizardError(error: unknown): WizardError {
  ...
  if (isAbortError(error)) {
    return {
      code: "timeout",
      message:
        "Модель не ответила за три минуты. Обычно помогает повторить попытку.",
    };
  }
